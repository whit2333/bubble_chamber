...
FakeSD::FakeSD(G4String name) : G4VSensitiveDetector(name)
{
  ...
  fhXY2_all    = fAnalysisManager->CreateH2(name+"/fhXY2_all","Y Vs X all; x [cm]; y [cm] ",     100,-2,2      ,100,-2,2);

  fhXY0_gamma    = fAnalysisManager->CreateH2(name+"/fhXY0_gamma","Y Vs X: gamma; x [cm]; y [cm] ",     100,-10,10,100,-10,10);
  fhXY1_gamma    = fAnalysisManager->CreateH2(name+"/fhXY1_gamma","Y Vs X: gamma; x [cm]; y [cm] ",     100,-5,5  ,100,-5,5);
  fhXY2_gamma    = fAnalysisManager->CreateH2(name+"/fhXY2_gamma","Y Vs X: gamma; x [cm]; y [cm] ",     100,-2,2  ,100,-2,2);

  fhPXY0_gamma    = fAnalysisManager->CreateH2(name+"/fhPXY0_gamma","P_{y} Vs P_{x}: gamma; P_{x} [MeV]; P_{y} [MeV] ", 100,-5.0,5.0, 100,-5.0,5.0);
  fhPXY1_gamma    = fAnalysisManager->CreateH2(name+"/fhPXY1_gamma","P_{y} Vs P_{x}: gamma; P_{x} [MeV]; P_{y} [MeV] ", 100,-2.0,2.0, 100,-2.0,2.0);
  fhPXY2_gamma    = fAnalysisManager->CreateH2(name+"/fhPXY2_gamma","P_{y} Vs P_{x}: gamma; P_{x} [MeV]; P_{y} [MeV] ", 100,-1.0,1.0, 100,-1.0,1.0);
...
G4bool FakeSD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
  ...
  if (aStep->GetPreStepPoint()->GetStepStatus() == fGeomBoundary)  {
    ...
    if (pdgcode == 22) { //photon
      ...
      fAnalysisManager->FillH2( fhPXY0_gamma, momentum.x(), momentum.y());
      fAnalysisManager->FillH2( fhPXY1_gamma, momentum.x(), momentum.y());
      fAnalysisManager->FillH2( fhPXY2_gamma, momentum.x(), momentum.y());
